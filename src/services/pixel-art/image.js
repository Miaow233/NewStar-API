                ...
                for (let x = 0; x < imgPixels.width; x++) {
                    ...
                    if (options.c < 0) {
                        if (options.c === -1) {
                            ...
                            imgPixels.data[i] = Math.round(
                                R + (R - avg) * options.c
                            )
                            imgPixels.data[i + 1] = Math.round(
                                G + (G - avg) * options.c
                            )
                            imgPixels.data[i + 2] = Math.round(
                                B + (B - avg) * options.c
                            )
                        }
                    } else {
...
export function download(options) {
    return new Promise((resolve) => {
        ...
        if (options.havePalette) {
            ...
            if (options.fillNums) {
                ...
                if (containColorKeys.length > 0) {
                    const colors = options.palette.filter((c) =>
        ...
        if (options.havePalette) {
            canvas.height =
                canvas.height + rows * rectH + (rows - 1) * rectM + padding
        }
        context.mozImageSmoothingEnabled = false
        context.webkitImageSmoothingEnabled = false
        context.imageSmoothingEnabled = false
        ...
        if (options.havePalette) {
            ...
            for (let row = 0; row < rows; row++) {
                for (let col = 0; col < cols; col++) {
                    ...
                    const y = Math.round(
                        height + padding * 2 + row * (rectH + rectM)
                    )
                    ...
                    contextTmp.font = `${Math.round(
                        (rectH / 2) * 0.8
                    )}px sans-serif`
                    contextTmp.fillStyle = colorInfo.isLightColor
                        ? '#000'
                        : '#fff'
                    contextTmp.fillText(
                        colorInfo.no,
